...
export default function Services() {
  ...
  const services = [
    { title: 'Gradient Mesh Websites', price: 'Rs 5,000', desc: 'Full gradient mesh themed websites.', features: ['Custom design', 'Responsive', 'Animations', 'SEO'] },
    { title: 'Gradient Mesh UI Kits', price: 'Rs 3,000', desc: 'Component libraries with gradient mesh aesthetics.', features: ['50+ components', 'Figma', 'Dark mode', 'Docs'] },
    { title: 'Gradient Mesh Branding', price: 'Rs 2,500', desc: 'Brand identities with gradient mesh design.', features: ['Logo', 'Colors', 'Guide', 'Assets'] },
  ]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {services.map((s, i) => (
          <div key={i} onClick={() => setSelected(i)} style={{ background: selected === i ? '#667eea10' : '#fff', borderRadius: '16px', border: '1px solid ' + (selected === i ? '#667eea' : '#667eea15'), padding: '24px', cursor: 'pointer', display: 'flex', flexDirection: 'column', transition: 'all 0.2s' }}>
            ...
            <Link href="/gradient-mesh/contact" style={{ display: 'block', textAlign: 'center', padding: '10px', background: '#667eea', color: '#fff', borderRadius: '8px', fontSize: '13px', fontWeight: 600, textDecoration: 'none' }}>Get Started</Link>
          </div>
        ))}
      </div>
