...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#eef0fb', fontFamily: '"Inter", sans-serif' }}>
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, height: '52px', background: '#fff', borderBottom: '1px solid #667eea30', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <span style={{ color: '#333', fontSize: '15px', fontWeight: 700 }}>Gradient Mesh</span>
        </div>
      </header>
      ...
      {sidebar && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 40 }}>
          ...
          <nav style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '260px', background: '#fff', borderRight: '1px solid #667eea20', paddingTop: '52px', boxShadow: '4px 0 16px rgba(0,0,0,0.1)' }}>
            <div style={{ padding: '16px' }}>
              <div style={{ textAlign: 'center', marginBottom: '24px', padding: '20px', background: '#667eea10', borderRadius: '12px', border: '1px solid #667eea20' }}>
                ...
                <p style={{ color: '#333', fontSize: '13px', fontWeight: 600 }}>Gradient Mesh</p>
              </div>
              {NAV.map(item => {
                const active = pathname === item.href
                return (
                  <Link key={item.href} href={item.href} onClick={() => setSidebar(false)} style={{ display: 'block', padding: '10px 16px', marginBottom: '4px', borderRadius: '10px', background: active ? '#667eea' : 'transparent', color: active ? '#fff' : '#333', textDecoration: 'none', fontSize: '14px', fontWeight: active ? 600 : 400, transition: 'all 0.2s' }}>
                    {item.label}
                  </Link>
                )
              })}
