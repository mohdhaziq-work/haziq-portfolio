...
export default function CyberpunkLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0a', fontFamily: '"Orbitron", sans-serif' }}>
      ...
      {/* Grid background */}
      <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', backgroundImage: `linear-gradient(${neon}08 1px, transparent 1px), linear-gradient(90deg, ${neon}08 1px, transparent 1px)`, backgroundSize: '40px 40px' }} />
      ...
      <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, height: '48px', background: '#0a0a0a', borderBottom: `1px solid ${neon}40`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <span style={{ color: '#4a6a7a', fontSize: '8px', letterSpacing: '2px' }}>SYS.{String(time).padStart(4, '0')}</span>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 40 }}>
          ...
          <nav style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '250px', background: '#0a0a0a', borderRight: `1px solid ${neon}30`, paddingTop: '48px' }}>
            <div style={{ padding: '16px' }}>
              <div style={{ textAlign: 'center', marginBottom: '24px', padding: '20px', background: `${neon}08`, border: `1px solid ${neon}20` }}>
                <div style={{ width: '44px', height: '44px', margin: '0 auto 8px', background: `${neon}20`, border: `1px solid ${neon}40`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <span style={{ color: neon, fontSize: '16px', fontWeight: 900, textShadow: `0 0 10px ${neon}60` }}>C</span>
                </div>
                <p style={{ color: neon, fontSize: '10px', fontWeight: 700, letterSpacing: '3px' }}>CYBER.STUDIO</p>
                <p style={{ color: '#4a6a7a', fontSize: '8px', letterSpacing: '1px' }}>Digital Design</p>
              </div>
              {NAV.map(item => {
                const active = pathname === item.href
                return (
                  <Link key={item.href} href={item.href} onClick={() => setSidebar(false)} style={{
                    display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', marginBottom: '4px',
                    ...
                    <span style={{ width: '4px', height: '4px', background: active ? neon : '#333', boxShadow: active ? `0 0 6px ${neon}` : 'none' }} />
                    {item.label}
                  </Link>
                )
              })}
