  ...
  return (
    <div style={{ padding: '24px 16px', maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ background: '#0a0a0a', border: `1px solid ${neon}30`, padding: '32px 24px', marginBottom: '24px' }}>
        <h1 style={{ color: neon, fontSize: '36px', fontWeight: 900, letterSpacing: '5px', textShadow: `0 0 20px ${neon}60`, marginBottom: '20px' }}>ABOUT US</h1>
        ...
        <p style={{ color: '#666', fontSize: '13px', lineHeight: 1.8 }}>Every pixel glows with purpose. Every interaction feels like jacking into the matrix.</p>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px', marginBottom: '24px' }}>
        {[
          { title: 'MISSION', desc: 'Push digital design boundaries with futuristic aesthetics.' },
          { title: 'VISION', desc: 'A web where every interface feels like a sci-fi control panel.' },
          { title: 'VALUES', desc: 'Innovation. Boldness. Neon. Always forward.' },
          { title: 'PROCESS', desc: 'Dark first. Add neon. Refine until it glows.' },
        ].map((item, i) => (
          <div key={i} style={{ background: '#0a0a0a', border: `1px solid ${neon}20`, padding: '18px' }}>
            ...
            <p style={{ color: '#4a6a7a', fontSize: '12px', lineHeight: 1.7 }}>{item.desc}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px', marginBottom: '24px' }}>
        {[
          ...
          <div key={i} style={{ background: `${neon}08`, border: `1px solid ${neon}20`, padding: '16px', textAlign: 'center' }}>
            <p style={{ color: neon, fontSize: '22px', fontWeight: 900, textShadow: `0 0 10px ${neon}60` }}>{s.value}</p>
