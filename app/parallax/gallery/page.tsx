...
export default function Gallery() {
  ...
  const works = [
    { title: 'Parallax Dashboard', cat: 'Web' },
    { title: 'Parallax Landing', cat: 'Web' },
    { title: 'Parallax Portfolio', cat: 'Web' },
    { title: 'Parallax UI Kit', cat: 'UI' },
    { title: 'Parallax App', cat: 'App' },
    { title: 'Parallax E-Commerce', cat: 'Web' },
    { title: 'Parallax Forms', cat: 'UI' },
    { title: 'Parallax Blog', cat: 'Web' },
    { title: 'Parallax Analytics', cat: 'App' },
  ]
  ...
  return (
    <div style={{ padding: '32px 20px', maxWidth: '900px', margin: '0 auto' }}>
      <div style={{ background: 'rgba(26,26,46,0.9)', borderRadius: '16px', border: '1px solid #667eea20', padding: '24px', marginBottom: '24px', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
        ...
        <p style={{ color: '#fff66', fontSize: '14px', marginTop: '8px' }}>Our parallax creations</p>
      </div>

      <div style={{ display: 'flex', gap: '8px', justifyContent: 'center', marginBottom: '24px' }}>
        {cats.map(c => (
          <button key={c} onClick={() => setActive(c)} style={{ padding: '8px 20px', borderRadius: '999px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: 600, background: active === c ? '#667eea' : 'rgba(26,26,46,0.9)', color: active === c ? '#fff' : '#fff88', transition: 'all 0.2s' }}>{c}</button>
        ))}
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '16px', marginBottom: '24px' }}>
        {filtered.map((w, i) => (
          <div key={i} style={{ background: 'rgba(26,26,46,0.9)', borderRadius: '12px', border: '1px solid #667eea15', overflow: 'hidden' }}>
