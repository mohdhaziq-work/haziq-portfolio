...
// ⚠️ YE CLIENT-SIDE PROTECTION HAI (FIREBASE RULES BHI HAIN)
const ADMIN_PASSWORD = 'haziq@2026'
// ==========================================================
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface px-6">
        <div className="elevated-card p-10 max-w-md w-full text-center">
          ...
          <p className="text-body-sm text-text-secondary mb-8">This area is private. Only the site owner can access it.</p>
          <input
            type="password"
            value={password}
            ...
            className="w-full px-4 py-3 rounded-lg border border-border bg-background text-text-primary text-body-md placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all mb-4"
          />
          <button onClick={handleLogin} className="btn-primary w-full py-3.5 justify-center">
            Unlock Dashboard
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-text-secondary text-body-sm">Loading data...</p>
          </div>
        ) : (
          <>
            {/* ==================== OVERVIEW TAB ==================== */}
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  {contacts.length === 0 ? (
                    <div className="elevated-card p-12 text-center">
                      <p className="text-text-tertiary text-body-sm">No contacts yet. They will appear when someone submits the contact form.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {contacts.slice(0, 5).map((contact) => (
                        <div key={contact.id} className="surface-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <p className="font-semibold text-text-primary text-body-sm">{contact.fullName}</p>
                              <p className="text-caption text-text-tertiary">{contact.businessName || 'No business'} • {contact.service}</p>
                            </div>
                          </div>
                          <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  {projects.filter((p) => !['delivered', 'cancelled'].includes(p.status)).length === 0 ? (
                    <div className="elevated-card p-12 text-center">
                      <p className="text-text-tertiary text-body-sm">No active projects. Add one from the Projects tab.</p>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {projects.filter((p) => !['delivered', 'cancelled'].includes(p.status)).map((project) => (
                        <div key={project.id} className="surface-card p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div>
                            <p className="font-semibold text-text-primary text-body-sm">{project.clientName}</p>
                            <p className="text-caption text-text-tertiary">{project.businessName} • {project.projectType} • {project.budget}</p>
                          </div>
                    ...
                    <div key={contact.id} className="elevated-card p-6">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                        <div className="bg-surface rounded-lg p-3">
                          ...
                          <p className="text-body-sm font-medium text-accent">{contact.instagramHandle}</p>
                        </div>
                        <div className="bg-surface rounded-lg p-3">
                          <p className="text-overline text-text-tertiary mb-1">Service</p>
                          <p className="text-body-sm font-medium text-text-primary">{contact.service}</p>
                        </div>
                        <div className="bg-surface rounded-lg p-3">
                          <p className="text-overline text-text-tertiary mb-1">Date</p>
                  ...
                  <div className="elevated-card p-6 border-2 border-accent/30">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                      <input
                        ...
                        className="w-full px-4 py-3 rounded-lg border border-border bg-background text-body-sm focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent"
                      />
                      <input
                        type="text"
                    ...
                    <div className="flex gap-3">
                      <button onClick={handleAddProject} className="btn-primary px-6 py-2.5 text-caption">
                        Add Project
                      </button>
                      <button onClick={() => setShowNewProject(false)} className="btn-outline px-6 py-2.5 text-caption">
                        Cancel
                      </button>
                    </div>
                  </div>
                    ...
                    <div key={project.id} className="surface-card p-6">
                      <div className="flex flex-col sm:flex-row justify-between gap-4 mb-4">
                        <div>
                          ...
                          <p className="text-body-sm text-text-secondary">{project.businessName || 'No business'} • {project.projectType} • {project.budget || 'No budget'}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <select
